...
import { SentinelDemo } from "@/components/sentinel-showcase-demo";
      ...
      <main id="welcome-main">
        ...
        <SentinelDemo>
          ...
          <div className="vl-concepts" id="how-it-works" tabIndex={-1} aria-labelledby="corners-title">
            ...
            <div className="vl-corner-grid" id="four-corners">{corners.map(({ id, number, title, icon: Icon, color, body }) => <article className={`vl-corner vl-${color}`} id={id} key={id} data-showcase-corner={id}><div className="vl-corner-top"><Icon size={28} strokeWidth={1.4} aria-hidden="true" /><span>{number}</span></div><h3>{title}</h3><p>{body}</p></article>)}</div>
          </div>
        ...
        <section className="vl-connections vl-wrap vl-section" id="connections" aria-labelledby="connections-title">
          <div className="vl-connections-heading">
            <div className="vl-section-heading"><p className="vl-kicker">02 / BUILD YOUR AVATAR</p><h2 id="connections-title">Build Your Avatar</h2></div>
            <div className="vl-build-description">
              <p>Start with <strong>seven basic inputs</strong> about your current financial situation.</p>
              <p>Planning a purchase? Open <strong>What If</strong> for five additional inputs and compare the scenario without changing your current picture.</p>
        ...
        <section className="vl-play-band" id="games" aria-labelledby="games-title">
          ...
          <section className="vl-welcome-explanation vl-wrap" aria-label="Explore the four concepts together">
