...
import { SHOT, campaignView, missionSummary, recurringOutflow } from "@/lib/debtbreaker-engine.js";
...
type Props={avatar:AvatarKind;strengths:CornerStrengths;hangar:Snapshot;visualState:AvatarVisualState;onExit:()=>void;onChallenges:()=>void;onClassic:()=>void};
...
export function DebtbreakerTurret({avatar,strengths,hangar,visualState,onExit,onChallenges,onClassic}:Props){
  const game=useDebtbreakerController(hangar,visualState.creditGrade),{state,world}=game;
  ...
  return <div className="sector-shell debtbreaker-shell db31-shell" data-phase={state.phase}>
    ...
    {state.phase==="briefing"&&<main className="dbt-briefing db31-briefing">
      <section><span className="eyebrow">AIM. PAY. PROTECT.</span><h3>Your income.<br/><em>Your line of defense.</em></h3>
        <p>Your avatar crews the turret. Hold the line against squads of five Debtonators weaving in from both gates. Debtonators explode when they reach your defenses; only their unpaid balance causes damage. Each squad shares one obligation: a hit can clear several small Debtonators without changing the shot value. Hit the friendly green tank to transfer income into your rear vault.</p>
      ...
      <aside className="dbt-scenario"><span className="eyebrow">FROM YOUR HANGAR</span><h4>{pictureName}</h4><p>Your selected picture supplies the money and loadout. Monthly income and obligations repeat for four game periods; unused income carries forward.</p>
        ...
        <Facts rows={[["Cash flow · firing",`${loadout.fireRate.toFixed(1)} shots / second`],["Collateral · starting armor",`${loadout.assetsMax}% each defense`],["Credit · radar reach",`${loadout.radar.toFixed(1)} field units · ${view.grade}`]]}/>
        ...
        <Button className="primary-action" disabled={!hasObligations} onClick={()=>game.start(pace)}><Crosshair size={18}/>Start Debtbreaker</Button>
        <Button variant="ghost" onClick={onExit}>Edit picture in hangar</Button>
        <p className="dbt-disclaimer">Simulation only. Game actions never change your hangar entries or move real money. APRs and account details are not entered, so loan payoff, interest and credit-score changes are not projected. Autofire and aim assist start off.</p>
      </aside>
    </main>}

    {(state.phase==="playing"||settling)&&<>
      <div className="db31-status" aria-label="Combat balances">
        <div><span>PERIOD / PACE</span><b>{state.period}/{state.maxPeriods} <small>{pace}</small></b><small>{world.current.drones.filter(d=>d.remaining>0&&!state.threats.find(t=>t.id===d.targetId)?.impacted).length} Debtonators approaching</small></div>
      ...
      <div className="db31-targets" aria-label="Inspect or aim at target">
        {liveTargets.map((t)=><button key={t.id} data-lane={t.lane} aria-pressed={state.selectedId===t.id} onClick={()=>game.select(t.id)}><span>{t.lane==="reserve"?"FRIENDLY · INCOME ONLY":`${t.lane==="credit"?"CREDIT":"LIVING COST"} · ${world.current.drones.filter(d=>d.targetId===t.id&&d.remaining>0).length} DEBTONATORS`}</span><b>{t.label}</b><strong>{t.lane==="reserve"?`${money(t.incomePaid)} deposited`:money(t.remaining)}</strong></button>)}
      </div>
      ...
      <details className="db31-details"><summary>Target details &amp; four pillars{selected?` — ${selected.label}`:""}</summary><div>
        <section><h4>{selected?.label??"Select a target"}</h4>{selected&&<Facts rows={selected.lane==="reserve"?[["Role","Friendly deposit tank"],["Deposited this period",money(selected.incomePaid)],["Funding","Income only; no savings loop"]]:[["Remaining",money(selected.remaining)],["Income paid",money(selected.incomePaid)],["Reserve paid",money(selected.reservePaid)],["Status",selected.impacted?"Breached":selected.remaining===0?"Paid":selected.overdue?"Overdue":selected.intercepted?"Passed interception":"Advancing"]]}/>}</section>
        <section><h4>Your hangar loadout · {pictureName}</h4><p><b>Cash Flow:</b> {loadout.fireRate.toFixed(1)} shots per second; each hit still uses up to {money(SHOT)}. <b>Capital:</b> your reserve balance funds protection. <b>Collateral:</b> each defense starts at {loadout.assetsMax}% armor. <b>Credit:</b> {view.grade}; the purple radar arc reaches {loadout.radar.toFixed(1)} field units. These use the existing Debtbreak combat mappings.</p><p>Assist follows only a target you explicitly choose below the field. Moving your aim releases the lock. Autofire fires at your reticle; it does not choose targets.</p><Button variant="ghost" onClick={()=>openInfo("budget")}>Hangar inputs</Button><Button variant="ghost" onClick={()=>openInfo("guide")}>Vi$ion guide</Button></section>
      </div></details>
    </>}
    ...
    {!settling&&["review","complete","gameover"].includes(state.phase)&&<main className="dbt-review db31-review">
      ...
      {state.review&&<div className="dbt-review-grid">
        <section><h4>Period {state.period} · income allocation</h4><Facts rows={[["Carried into this period",money(state.review.carriedIncome)],["New income received",money(state.review.incomeReceived)],["Income paid to obligations",money(state.review.incomeSpent-state.review.reserveDeposited)],["Income deposited to reserves",money(state.review.reserveDeposited)],["Unallocated income at checkpoint",money(state.review.unallocatedIncome)]]}/><p className="db31-note">Unallocated income remains yours. It carries forward; it is not lost at the next paycheck.</p></section>
        <section><h4>Period {state.period} · obligations &amp; vault</h4><Facts rows={[["Opening reserve balance",money(state.review.startingReserves)],["Reserves withdrawn for protection",money(state.review.reserveSpent)],["Ending reserve balance",money(state.review.endingReserves)],["Unpaid this period",money(state.review.arrears)],["Principal / interest split","Not modeled · APRs not entered"]]}/><p className="db31-note">Debt payments satisfy this period’s entered payment amount. They are not treated as entirely principal, and no payoff balance is invented.</p></section>
      ...
      {state.phase==="review"&&<>
        <section className="db31-note"><h4>Continue this picture or change it</h4><p>Your monthly amounts stay fixed through the mission. Defenses keep their damage between periods. Asset sales, equipment prices and financing are not inferred from your hangar totals.</p><Button variant="outline" onClick={()=>leave(onExit)}>Edit picture in hangar</Button></section>
