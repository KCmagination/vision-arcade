...
import { SHOT, incomeFire, reserveFire, tickCampaign } from './debtbreaker-engine.js';
...
export function defenseContact(lane, slot = 0, defenseIndex = 0) {
  const angle = (lane === 'credit' ? -1 : 1) * (.4 + slot * .135);
...
function syncFormation(world, preferredId = null) {
  if(world.formationPeriod!==world.ledger.period || !world.drones.length){
    ...
    world.drones=world.ledger.threats.filter(t=>t.lane!=='reserve').flatMap(t=>
      Array.from({length:SQUAD_SIZE},(_,slot)=>({id:`${t.id}:drone:${slot}`,targetId:t.id,slot,
        remaining:Math.floor(t.remaining/SQUAD_SIZE)+(slot<t.remaining%SQUAD_SIZE?1:0)})));
  }
...
function step(world, dt) {
  ...
  if((world.firing||world.autoFire||world.pendingShot)&&world.cooldown<=0&&balance>0&&Math.abs(wrapAngle(desired-world.angle))<.075){
    ...
    world.cooldown=world.ledger.hangar?1/world.ledger.hangar.loadout.fireRate:CADENCE;world.pendingShot=false;world.shots++;event(world,'shot',TURRET);
  }
  const remaining=[];
  for(const bullet of world.bullets){
    ...
    if(best){
      ...
      const after=bullet.source==='reserve'?reserveFire(before,best.target.id,SHOT):incomeFire(before,best.target.id,SHOT);
