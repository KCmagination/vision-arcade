...
function makeThreats(state) {
  ...
  return [
    threat(`living-${state.period}`, 'living', state.hangar ? 'Living expenses' : 'Living + upkeep', living, .75),
    ...(state.hangar?.monthlyDebtPayments > 0 ? [threat(`payments-${state.period}`, 'credit', 'Debt payments', state.hangar.monthlyDebtPayments, .9, true)] : []),
...
export function createHangarCampaign(snapshot, grade = null) {
...
export function beginCampaign(input) {
  if (input.phase !== 'briefing') return clone(input);
