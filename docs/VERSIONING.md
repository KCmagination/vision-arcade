...
The public guard allows only exact reviewed uses of the existing collateral ratio output in two gameplay files and one synthetic fixture. This is not permission to publish its calculation or any scoring calibration. Guard regression checks reject other uses, including formulas inside those same files.
